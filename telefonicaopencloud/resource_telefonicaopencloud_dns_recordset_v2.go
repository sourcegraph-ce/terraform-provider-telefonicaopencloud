...
func resourceDNSRecordSetV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud DNS record set: %s", err)
	}
	...
	log.Printf("[DEBUG] Created TelefonicaOpenCloud DNS record set %s: %#v", n.ID, n)
...
func resourceDNSRecordSetV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating TelefonicaOpenCloud DNS  record set: %s", err)
	}
...
func resourceDNSRecordSetV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting TelefonicaOpenCloud DNS  record set: %s", err)
	}
...
func waitForDNSRecordSet(dnsClient *golangsdk.ServiceClient, zoneID, recordsetId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud DNS record set (%s) current status: %s", recordset.ID, recordset.Status)
