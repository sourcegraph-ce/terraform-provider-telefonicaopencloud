...
func resourceVBSBackupV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating Telefonicaopencloud VBS Backup: %s", err)
	}
...
func resourceVBSBackupV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	vbsClient, err := config.vbsV2Client(GetRegion(d, config))
	if err != nil {
		return fmt.Errorf("Error creating Telefonicaopencloud vbs: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error deleting Telefonicaopencloud VBS Backup: %s", err)
	}
...
func waitForBackupDelete(client *golangsdk.ServiceClient, backupID string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		if r.Status != "deleting" {
			...
			if err != nil {
				if _, ok := err.(golangsdk.ErrDefault404); ok {
					log.Printf("[INFO] Successfully deleted Telefonicaopencloud VBS backup %s", backupID)
					return r, "deleted", nil
				}
