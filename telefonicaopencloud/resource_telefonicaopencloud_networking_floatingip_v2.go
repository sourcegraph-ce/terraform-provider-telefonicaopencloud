...
func resourceNetworkFloatingIPV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Waiting for TelefonicaOpenCloud Neutron Floating IP (%s) to become available.", floatingIP.ID)
...
func resourceNetworkFloatingIPV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting TelefonicaOpenCloud Neutron Floating IP: %s", err)
	}
...
func waitForFloatingIPActive(networkingClient *golangsdk.ServiceClient, fId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud Neutron Floating IP: %+v", f)
...
func waitForFloatingIPDelete(networkingClient *golangsdk.ServiceClient, fId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete TelefonicaOpenCloud Floating IP %s.\n", fId)

		f, err := floatingips.Get(networkingClient, fId).Extract()
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted TelefonicaOpenCloud Floating IP %s", fId)
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud Floating IP %s still active.\n", fId)
		return f, "ACTIVE", nil
	}
}
