...
func bmsTagsCreate(client *golangsdk.ServiceClient, server_id string) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud Tags: %s", err)
	}
	return nil
}
...
func resourceComputeBMSInstanceV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud bms client: %s", err)
	}
	err = bmsTagsCreate(bmsClient, d.Id())
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud bms tag: %s", err)
	}
