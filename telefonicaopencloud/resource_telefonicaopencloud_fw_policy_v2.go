...
func resourceFWPolicyV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Read TelefonicaOpenCloud Firewall Policy %s: %#v", d.Id(), policy)
	...
	if err := d.Set("rules", policy.Rules); err != nil {
		return fmt.Errorf("[DEBUG] Error saving rules to state for TelefonicaOpenCloud firewall policy (%s): %s", d.Id(), err)
	}
