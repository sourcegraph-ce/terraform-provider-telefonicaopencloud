...
func resourceFWFirewallGroupV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Read TelefonicaOpenCloud Firewall group %s: %#v", d.Id(), firewall_group)
	...
	if err := d.Set("ports", firewall_group.PortIDs); err != nil {
		return fmt.Errorf("[DEBUG] Error saving ports to state for TelefonicaOpenCloud firewall group (%s): %s", d.Id(), err)
	}
