...
func resourceDNSZoneV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud DNS zone: %s", err)
	}
	...
	log.Printf("[DEBUG] Created TelefonicaOpenCloud DNS Zone %s: %#v", n.ID, n)
...
func resourceDNSZoneV2Update(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error updating TelefonicaOpenCloud DNS Zone: %s", err)
	}
...
func resourceDNSZoneV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting TelefonicaOpenCloud DNS Zone: %s", err)
	}
...
func waitForDNSZone(dnsClient *golangsdk.ServiceClient, zoneId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud DNS Zone (%s) current status: %s", zone.ID, zone.Status)
