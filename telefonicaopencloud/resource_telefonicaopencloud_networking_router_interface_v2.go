...
func resourceNetworkingRouterInterfaceV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud Neutron router interface: %s", err)
	}
...
func resourceNetworkingRouterInterfaceV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error retrieving TelefonicaOpenCloud Neutron Router Interface: %s", err)
	}
...
func resourceNetworkingRouterInterfaceV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = stateConf.WaitForState()
	if err != nil {
		return fmt.Errorf("Error deleting TelefonicaOpenCloud Neutron Router Interface: %s", err)
	}
...
func waitForRouterInterfaceActive(networkingClient *golangsdk.ServiceClient, rId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud Neutron Router Interface: %+v", r)
...
func waitForRouterInterfaceDelete(networkingClient *golangsdk.ServiceClient, d *schema.ResourceData) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] Attempting to delete TelefonicaOpenCloud Router Interface %s.", routerInterfaceId)
		...
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted TelefonicaOpenCloud Router Interface %s", routerInterfaceId)
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud Router Interface %s is still active.", routerInterfaceId)
		return r, "ACTIVE", nil
	}
}
