...
func resourceNetworkingPortV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] Waiting for TelefonicaOpenCloud Neutron Port (%s) to become available.", p.ID)
...
func waitForNetworkPortActive(networkingClient *golangsdk.ServiceClient, portId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud Neutron Port: %+v", p)
...
func waitForNetworkPortDelete(networkingClient *golangsdk.ServiceClient, portId string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		log.Printf("[DEBUG] Attempting to delete TelefonicaOpenCloud Neutron Port %s", portId)

		p, err := ports.Get(networkingClient, portId).Extract()
		if err != nil {
			if _, ok := err.(golangsdk.ErrDefault404); ok {
				log.Printf("[DEBUG] Successfully deleted TelefonicaOpenCloud Port %s", portId)
		...
		log.Printf("[DEBUG] TelefonicaOpenCloud Port %s still active.\n", portId)
		return p, "ACTIVE", nil
	}
}
