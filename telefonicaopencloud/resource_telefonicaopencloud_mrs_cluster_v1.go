...
func resourceClusterV1Create(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating TelefonicaOpenCloud MRS client: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("Error retrieving TelefonicaOpenCloud Subnet: %s", err)
	}
...
func resourceClusterV1Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error deleting TelefonicaOpenCloud Cluster: %s", err)
	}
