		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckASV1PolicyExists("telefonicaopencloud_as_policy_v1.hth_as_policy", &asPolicy),
				),
			},
		},
	})
...
func testAccCheckASV1PolicyDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_as_policy_v1" {
			continue
		}
...
resource "telefonicaopencloud_networking_secgroup_v2" "secgroup" {
  name        = "terraform"
...
resource "telefonicaopencloud_as_group_v1" "hth_as_group"{
  ...
  security_groups = [
    {id = "${telefonicaopencloud_networking_secgroup_v2.secgroup.id}"},
  ]
  vpc_id = "%s"
}

resource "telefonicaopencloud_as_policy_v1" "hth_as_policy"{
  scaling_policy_name = "terraform"
  scaling_group_id = "${telefonicaopencloud_as_group_v1.hth_as_group.id}"
